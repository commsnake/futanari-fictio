...
Welcome to the Futanari Writing Repository. As an AI Agent working within this project, you must adhere to the following master workflow constraints and behavioral guidelines.
...
*   You must immediately run `python3 scripts/sync_to_master.py` to ensure the updates are copied to the `master_template/` folder and that any domain-specific terminology (like "Futanari") is replaced with generic placeholders (like `[Genre/Theme]`).